...
3. **Actualización Final y Handoff:** Al terminar un paso, y SOLO CON APROBACIÓN EXPLÍCITA, el Orquestador actualiza los JSON de tareas. **REGLA ABSOLUTA:** El Orquestador tiene PROHIBIDO editar los archivos de estado `state/session_handoff.md`, `state/pendings.md` Y de la carpeta `diagrams/`, siendo estos responsabilidad exclusiva del `session_verifier` durante el cierre de sesión. No obstante, **el archivo de progreso `state/progress.md` DEBE ser editado por cada agente especialista al concluir su respectivo turno** para registrar su relevo de fase (decisiones tomadas, recursos estudiados y asignación de `Último Turno` y `Siguiente Turno`).

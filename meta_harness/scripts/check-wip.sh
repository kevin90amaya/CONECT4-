...
BOB_JSON="tareas/bob_feature_list.json"
...
if [[ ! -f "$BOB_JSON" ]]; then
    echo "ERROR: No se encuentra el archivo $BOB_JSON"
    exit 1
fi
...
if ! bob_count=$(jq -e '[.features[] | select(.status == "in_progress" or .status == "active")] | length' "$BOB_JSON" 2>/dev/null); then
    echo "ERROR: $BOB_JSON no es un JSON válido o no se pudo leer."
    exit 1
fi

total_active=$((meta_count + bob_count))

if [[ "$total_active" -gt 1 ]]; then
    echo "FAIL: Se detectó una violación de la regla WIP=1."
    echo "Hay $total_active tareas activas en total ($meta_count en meta-harness, $bob_count en bob-harness)."
